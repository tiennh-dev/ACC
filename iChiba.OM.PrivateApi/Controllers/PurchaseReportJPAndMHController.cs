...
namespace iChiba.OM.PrivateApi.Controllers
{
    ...
    public class PurchaseReportJPAndMHController : BaseController
    {
        private readonly IPurchaseReportAppService purchaseReportService;
        private readonly IHostingEnvironment environment;
        public PurchaseReportJPAndMHController(ILogger<PurchaseReportController> logger, IPurchaseReportAppService purchaseReportService, IHostingEnvironment environment) : base(logger)
        {
        ...
        public async Task<IActionResult> GetPurchaseReportJpAndMh()
        {
            try
            {
                var response = await purchaseReportService.GetPurchaseReportJpAndMh();
        ...
        [ProducesResponseType((int)HttpStatusCode.Forbidden)]
        [ProducesResponseType((int)HttpStatusCode.OK, Type = typeof(BaseResponse))]
        public async Task<IActionResult> GetPurchaseReportDataJpAndM(PurchaseReportMercariRequest request)
        {
            try
            {
                var data = await purchaseReportService.GetPurchaseReportDataJpAndMh(request);
                return Ok(data);
            }
            catch (Exception ex)
            {
