...
namespace iChiba.OM.PrivateApi.Controllers
{
    ...
    public class PurchaseReportAucController : BaseController
    {
        private readonly IPurchaseReportAppService purchaseReportService;
        private readonly IHostingEnvironment environment;
        public PurchaseReportAucController(ILogger<PurchaseReportController> logger, IPurchaseReportAppService purchaseReportService, IHostingEnvironment environment) : base(logger)
        {
        ...
        public async Task<IActionResult> GetPurchaseReportAuc()
        {
            try
            {
                var response = await purchaseReportService.GetPurchaseReportAUC();
        ...
        [ProducesResponseType((int)HttpStatusCode.Forbidden)]
        [ProducesResponseType((int)HttpStatusCode.OK, Type = typeof(BaseResponse))]
        public async Task<IActionResult> GetPurchaseReportDataAuc(PurchaseReportMercariRequest request)
        {
            try
            {
                var data = await purchaseReportService.GetPurchaseReportDataAuc(request);
                return Ok(data);
            }
            catch (Exception ex)
            {
