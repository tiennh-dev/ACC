...
{
    public class PurchaseWaitingController : BaseController
    {
        ...
        public PurchaseWaitingController(ILogger<PurchaseBuyForYouController> logger,
        ...
        public async Task<IActionResult> Export(OrderBuyForYouListRequest request)
        {
            try
            {
                ...
                {
                    orderLists = await orderBuyForYouAppService.ExportPurchaseBuyForYou(request);
                }
                if (orderLists == null)
                {
                ...
                using (var package = new ExcelPackage(fileInfo, true))
                {
                    ...
                    foreach (var item in orderLists.Data)
                    {
                        ...
                        workSheet.Cells[irow, 16].Value = item.ShippingUnitGlobal;
                        workSheet.Cells[irow, 17].Value = tempPrice;
                        workSheet.Cells[irow, 18].Value = item.BidAccount;
                        workSheet.Cells[irow, 19].Value = item.FullName;
                        workSheet.Cells[irow, 20].Value = item.EmpployeeSupport;
                        workSheet.Cells[irow, 21].Value = item.Tracking;
                        workSheet.Cells[irow, 22].Value = "";
                        workSheet.Cells[irow, 23].Value = "";
                        workSheet.Cells[irow, 24].Value = "";
                        irow += 1;
                    ...
                    workSheet.Cells["A" + (orderLists.Data.Count + 8) + ":X" + (orderLists.Data.Count + 8)].Style.Fill.PatternType = ExcelFillStyle.Solid;
                    workSheet.Cells["A" + (orderLists.Data.Count + 8) + ":X" + (orderLists.Data.Count + 8)].Style.Fill.BackgroundColor.SetColor(System.Drawing.Color.Yellow);
                    ...
                    workSheet.Cells[orderLists.Data.Count + 8, 12].Formula = "SUM(L8:L" + (orderLists.Data.Count + 7) + ")";
                    workSheet.Cells[orderLists.Data.Count + 8, 17].Formula = "SUM(Q8:Q" + (orderLists.Data.Count + 7) + ")";
                    workSheet.Cells["A7:X7"].Style.Font.Size = 11;
                    workSheet.Cells["A7:X7"].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
                    workSheet.Cells["A7:X7"].Style.VerticalAlignment = ExcelVerticalAlignment.Center;
                    workSheet.Cells["A7:X" + (orderLists.Data.Count + 8)].Style.Border.Top.Style = ExcelBorderStyle.Thin;
                    workSheet.Cells["A7:X" + (orderLists.Data.Count + 8)].Style.Border.Right.Style = ExcelBorderStyle.Thin;
                    workSheet.Cells["A7:X" + (orderLists.Data.Count + 8)].Style.Border.Left.Style = ExcelBorderStyle.Thin;
                    workSheet.Cells["A7:X" + (orderLists.Data.Count + 8)].Style.Border.Bottom.Style = ExcelBorderStyle.Thin;
        ...
        public async Task<IActionResult> SplitOrderWaitForBuy(OrderSplitRequest request)
        {
            try
            {
                ...
                if (response.Status)
                {
                    var requestWfSplitOrders = response.Data.Select(item => new OrderWVModel()
                    {
                        Id = item,
                    ...
                    using (var scope = serviceProvider.CreateScope())
                    {
                        ...
                        requestWfSplitOrders.ForEach(item =>
                        {
        ...
        [ProducesResponseType((int)HttpStatusCode.Forbidden)]
        [ProducesResponseType((int)HttpStatusCode.OK, Type = typeof(BaseEntityResponse<IList<int>>))]
        public async Task<IActionResult> OrderDetailUpdateStatus(OrderDetailUpdateStatusRequest request)
        {
            try
            {
                var response = await orderBuyForYouAppService.OrderDetailUpdateStatus(request);
                ...
                await AutoApproveOrder(request.OrderId);
        ...
        [ProducesResponseType((int)HttpStatusCode.Forbidden)]
        [ProducesResponseType((int)HttpStatusCode.OK, Type = typeof(BaseResponse))]
        public async Task<IActionResult> UpdateShippingFreeOrderDetail(int idOrderDetail,int? shippingFree)
        {
            try
            {
                var response = await orderBuyForYouAppService.UpdateShippingFreeForOrderDetail(idOrderDetail, shippingFree);
                return Ok(response);
            }
            catch (Exception ex)
            {
        ...
        private async Task AutoApproveOrder(int orderId)
        {
            var orderDetailStatus = await orderBuyForYouAppService.GetOrderDetailStatusByOrderId(orderId);

            if (orderDetailStatus == OrderDetailStatus.Processing)
            {
                return;
            }
            ...
            using (var scope = serviceProvider.CreateScope())
            {
                ...
                if (orderDetailStatus == OrderDetailStatus.Bought)
                {
                    var requestWf = new OrderWVModel()
                    {
                        Id = orderId,
                        Message = "Hệ thống tự động duyệt mua hàng dựa trên trạng thái mua hàng sản phẩm",
                        Action = "TỰ ĐỘNG DUYỆT MUA HÀNG"
                    };
                    var result = orderWorkflowAppServiceTemp.BuyProduct(requestWf).Result;

                    if (!result.Succeeded)
                    {
                        throw new ErrorCodeException(ErrorCodeDefine.ORDER_DETAIL_UPDATE_STATUS_CANT_AUTO_APPROVE_BUY_PRODUCT);
                    }
                }
                else if (orderDetailStatus == OrderDetailStatus.CantBuy)
                {
                    var requestWf = new OrderWVModel()
                    {
                        Id = orderId,
                        Action = "Y/C HUỶ ĐƠN",
                        Message = "Hệ thống tự động duyệt huỷ đơn dựa trên trạng thái mua hàng sản phẩm"
                    };
                    var result = orderWorkflowAppServiceTemp.RequestCancelOrder(requestWf).Result;

                    if (!result.Succeeded)
                    {
                        throw new ErrorCodeException(ErrorCodeDefine.ORDER_DETAIL_UPDATE_STATUS_CANT_AUTO_CANCEL_ORDER);
                    }
                }
            }
        }
        ...
        [ProducesResponseType((int)HttpStatusCode.Forbidden)]
        [ProducesResponseType((int)HttpStatusCode.OK, Type = typeof(BaseEntityResponse<OrderDetailApp>))]
        public async Task<IActionResult> GetOrderById(int IdOrder)
        {
            try
            {
                var response = await orderAppService.GetorderById(IdOrder);
                return Ok(response);
            }
            catch (Exception ex)
            {
