...
{
    public class WaitingOrderAutionDgsController : BaseController
    {
        ...
        public WaitingOrderAutionDgsController(ILogger<OrderAutionDgsController> logger,
        ...
        public async Task<IActionResult> Export(OrderListRequest request)
        {
            try
            {
                ...
                using (var package = new ExcelPackage(fileInfo, true))
                {
                    ...
                    workSheet.Cells["D2"].Value = "Đơn Chờ Tất Toán DGC " + String.Format("{0:dd/MM/yyyy}", time);
