...
{
    public class WaitingDepositeController : BaseController
    {
        ...
        private const string waitingDepositeStatus = "0";

        public WaitingDepositeController(ILogger<DepositsController> logger,
        ...
        public async Task<IActionResult> GetJTable(DepositsListJTableModel request)
        {
            try
            {
                ...
                appserviceRequest.DepositSt = waitingDepositeStatus;
                appserviceRequest.PayStatus = (int)PayStatus.Cancel;
                appserviceRequest.DepositeType = DepositType.REFUND;
                var response = await depositsAppService.GetListJTableWaitingDeposite(appserviceRequest);
        ...
        public async Task<IActionResult> Export(DepositsListRequest request)
        {
            try
            {
                ...
                request.DepositSt = waitingDepositeStatus;
                request.PayStatus = (int)PayStatus.Cancel;
                request.DepositeType = DepositType.REFUND;
                var data = await depositsAppService.GetsDataExportWaitingDeposite(request);
                if (data == null)
                {
