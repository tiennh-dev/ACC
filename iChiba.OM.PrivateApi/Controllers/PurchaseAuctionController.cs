...
{
    public class PurchaseAuctionController : BaseController
    {
        ...
        public PurchaseAuctionController(ILogger<PurchaseAuctionController> logger,
        ...
        [ProducesResponseType((int)HttpStatusCode.Forbidden)]
        [ProducesResponseType((int)HttpStatusCode.OK, Type = typeof(BaseResponse))]
        public async Task<IActionResult> UpdateNote(OrderUpdateNoteRequest request)
        {
            try
            {
                var response = await orderAppService.UpdateNote(request);
