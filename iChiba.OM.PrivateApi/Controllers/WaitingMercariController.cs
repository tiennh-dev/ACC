...
{
    public class WaitingMercariController : BaseController
    {
        ...
        public WaitingMercariController(ILogger<OrderBuyForYouController> logger,
            ...
            try
            {
                ...
                string[] arrString = new string[]
               {
                ...
                using (var package = new ExcelPackage(fileInfo, true))
                {
                    ...
                    workSheet.Cells["D2"].Value = "Chờ tất toán mercari " + String.Format("{0:dd/MM/yyyy}", time);
                    ...
                    foreach (var item in data.Data)
                    {
                        ...
                        try
                        {
                            workSheet.Cells[irow, 3].Formula = "HYPERLINK(\"" + item.Link + "\",\"" + item.ProductName + "\")";
                        }
                        catch (Exception) { }
                        try
                        {
                            workSheet.Cells[irow, 4].Hyperlink = new Uri(item.Link, UriKind.Absolute);
                        }
                        catch (Exception) { }
